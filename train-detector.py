...
def read_data(root_dir, gt_file):
    data = []
    ...
    with open(gt_file, 'r') as f:
        ...
        for lid, line in enumerate(lines):
            ...
            for idx, token in enumerate(objs):
                if token == LABEL:
                    try:
                        lp = [float(x) for x in objs[idx+1:idx+9]]
                        lp = lp[::2] + lp[1::2]
                        lp = [4] + lp
...
#             Data.append([I,L[0]])
    ...
    Data = read_data('/workspace/dataset/lpd_poly', 'lpd_poly_groundtruth_train.txt') #'/workspace/dataset/lpd_poly/lpd_poly_groundtruth_train.txt')
